...
st.set_page_config(page_title="Boston 311 Needle Requests", layout="wide")
...
selected_years = st.sidebar.multiselect("Years", available_years, default=default_years)
...
st.sidebar.metric("Total Requests", f"{len(df):,}")
...
with col_charts:
    ...
    monthly = (
        df.groupby([df["dt"].dt.year.rename("year"), df["dt"].dt.month.rename("mo")]).size().reset_index(name="count")
    )
    ...
    fig_hour.update_layout(height=200, margin=dict(l=0, r=0, t=10, b=0), xaxis_title="Hour", yaxis_title="Requests")
    st.plotly_chart(fig_hour, use_container_width=True)

# -- Bottom row --
col_hoods, col_zips = st.columns(2)
